...
class SparseConvFeaturesSortedImplicitGemm final : public SparsePrimitive {
  public:
    ...
    jvp(const std::vector<mx::array>& primals,
        ...
        auto has_tangent = false;
        for (int index = 0; index < int(argnums.size()); ++index) {
            if (argnums[index] != 0 && argnums[index] != 1) {
                continue;
            }
            auto component = make_sparse_conv_features_sorted_implicit_gemm(
                argnums[index] == 0 ? tangents[index] : primals[0],
                argnums[index] == 1 ? tangents[index] : primals[1],
                SparseConvSortedImplicitGemmView{
                    primals[2], primals[3], primals[4], primals[5]
                },
                SparseRelationEdges{primals[6], primals[7], primals[8]},
                SparseRelationContract{
                    primals[9], shape_.out_capacity, shape_.n_kernels
                },
                SparseRelationExecutionViews{
                    SparseRelationCSRView{primals[10], primals[10]},
                    SparseRelationCSRView{
                        plan_.in_row_offsets, plan_.in_edge_ids
                    },
                    SparseRelationCSRView{
                ...
                shape_.store_sorted != 0
            );
            out = has_tangent ? mx::add(out, component, stream()) : component;
            has_tangent = true;
        }
        return {out};
    }
...
mx::array make_sparse_conv_features_sorted_implicit_gemm(
    ...
    auto stream = sparse_conv_implicit_gemm_stream(
        feats, weights, sorted_view.sorted_out_in_map
    );
    auto device = sparse_exec_device();
    auto primitive = std::make_shared<SparseConvFeaturesSortedImplicitGemm>(
        stream,
        shape,
        SparseConvPlan{
            views.input_csr.row_offsets,
    ...
    auto inputs = std::vector<mx::array>{
        ...
        mx::contiguous(sorted_view.sorted_out_in_map, false, device),
        mx::contiguous(sorted_view.sorted_kv_out_in_map, false, device),
        mx::contiguous(sorted_view.reorder_rows, false, device),
        mx::contiguous(sorted_view.tile_masks, false, device),
        edges.in_rows,
        edges.out_rows,
        edges.kernel_ids,
        contract.counts,
